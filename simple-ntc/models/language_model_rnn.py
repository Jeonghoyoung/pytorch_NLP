...
class LanguageModel(nn.Module):
    ...
    def forward(self, x):
        # |x| = (batch_size, length)
        x = self.emb(x)
        # |x| = (ba†ch_size, length, word_vec_dim)
        x, (h, c) = self.rnn(x)
        # |x| = (ba†ch_size, length, hidden_size)
        x = self.out(x)
        # |x| = (ba†ch_size, length, vpcab_size)
    ...
    def search(self, batch_size=64, max_length=255):
        ...
        while is_undone.sum() > 0 and len(indice) < max_length:
            x = self.emb(x)
            # |emb_t| = (ba†ch_size, 1, word_vec_dim)
            x, (h, c) = self.rnn(x, (h, c)) if h is not None and c is not None else self.rnn(x)
            # |x| = (ba†ch_size, 1, hidden_size)
            y_hat = self.log_softmax(x)
            # |y_hat| = (ba†ch_size, 1, output_size)
